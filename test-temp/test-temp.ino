...
// ---------------------------------------------------------------------
    ...
    armError_t err = Objenious.Init(&Serial);
    if(err != ARM_ERR_NONE)
    {
    ...
    err = Objenious.UpdateConfig();

    if(err != ARM_ERR_NONE)
    {
      mySerial.println(err);
        digitalWrite(LED_ERROR, HIGH);
        mySerial.println("Update error"); // Debug
    }
    else
    {
        digitalWrite(LED_OK, HIGH);
        mySerial.println("Update OK"); // Debug
    }
...
void loop()
{
  ...
  msgg[2] = (byte) temp;
  mySerial.print("SEnd : ");
  mySerial.println(Objenious.Send("toto", sizeof("toto")));
//  Objenious.Receive(data, sizeof(data), 4000);      // Receive the packet from server if it is available.
//  for (byte i = 0; i < sizeof(data); i++)        
//  {
//    mySerial.print(data[i],HEX);                    //We serial print it byte by byte
//    data[i] = 0;                                    //We clean the data buffer byte by byte.
//  }
//  mySerial.println();
  
  //mySerial.println(Objenious.Send(msgg, sizeof(msgg)));               // Send the temp to Objenious network
